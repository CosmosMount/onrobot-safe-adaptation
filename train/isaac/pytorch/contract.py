"""Tensor conversion and joint-order checks for the Isaac adapter."""

import numpy as np
import torch

from train.core.base import ACTION_SPEC, DEFAULT_JOINT_POSITION, JOINT_NAMES
...
def sdk_joint_indices(source_names, device=None):
    ...
    if duplicates:
        raise ValueError(
            f"Duplicate joints in Isaac articulation: {duplicates}"
        )
    ...
    unexpected = [name for name in normalized if name not in JOINT_NAMES]
    if unexpected or len(normalized) != len(JOINT_NAMES):
        raise ValueError(
            "Isaac articulation must expose exactly the 12 SDK Go2 joints; "
            f"unexpected={unexpected}, count={len(normalized)}"
        )
    ...
    if torch.unique(indices).numel() != len(JOINT_NAMES):
        raise RuntimeError("Isaac SDK joint mapping is not one-to-one")
    return indices
