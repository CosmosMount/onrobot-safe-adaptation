...
from rl_x.algorithms.droq.flax.policy import get_policy
from rl_x.algorithms.droq.flax.critic import get_critic
from rl_x.algorithms.droq.flax.entropy_coefficient import EntropyCoefficient
from rl_x.algorithms.droq.flax.replay_buffer import ReplayBuffer
from rl_x.algorithms.droq.flax.rl_train_state import RLTrainState
...
class DroQ_QSafe:
    def __init__(
        ...
        self.key, policy_key, critic_key, dropout_key, entropy_coefficient_key = (
        ...
        if self._host_project_actions is not None and not self._projector_is_jax:
            rlx_logger.warning(
                ...
                "will be projected on the host, while differentiable DroQ actor "
        ...
        critic_params = self.critic.init(
            {"params": critic_key, "dropout": dropout_key}, state, action
        )
        critic_target_params = jax.tree.map(lambda value: value.copy(), critic_params)
    ...
    def _build_task_update(self):
        ...
        def update(
            ...
            qsafe_params: flax.core.FrozenDict,
            nu: float,
            ...
            def critic_loss_fn(
                ...
                next_action_mean, next_action_logstd = self.policy.apply(
                    stop_gradient(policy_state.params), next_state
                )
                ...
                next_action_pretanh = (
                    ...
                    + next_action_std
                    * jax.random.normal(
                        sample_key, shape=next_action_mean.shape
                    )
                )
                ...
                alpha = stop_gradient(
                    self.entropy_coefficient.apply(
                ...
                target = reward + self.gamma * (1.0 - terminated) * (
                    jnp.min(next_q_target) - alpha * next_log_prob
                )
                q_values = self.critic.apply(
                    critic_params,
                    state,
                    action,
                    rngs={"dropout": dropout_current_key},
                )
                q_loss = (q_values - target) ** 2
                return q_loss, {"loss/q_loss": q_loss}

            def policy_entropy_loss_fn(
                policy_params,
                ...
                action_pretanh = (
                    action_mean
                    + action_std
                    * jax.random.normal(sample_key, shape=action_mean.shape)
                )
                policy_action = jnp.tanh(action_pretanh)
                current_action = self._jax_project_actions(
                    raw_state[None, :], policy_action[None, :]
                )[0]
                current_log_prob = squashed_gaussian_log_probability(
                    action_pretanh, action_mean, action_logstd
                )
                entropy = stop_gradient(-current_log_prob)
                q_values = self.critic.apply(
                    stop_gradient(critic_state.params),
                    state,
                    current_action,
                    rngs={"dropout": dropout_key},
                )
                min_q = jnp.min(q_values)
            ...
            vmapped_policy_loss = jax.vmap(
                policy_entropy_loss_fn,
                in_axes=(None, None, 0, 0, 0, 0),
                out_axes=0,
            )
            safe_mean = lambda value: (
                jnp.mean(value) if value is not None else value
            )
            mean_critic_loss = lambda *args: tree.map_structure(
                safe_mean, vmapped_critic_loss(*args)
            )
            mean_policy_loss = lambda *args: tree.map_structure(
                safe_mean, vmapped_policy_loss(*args)
            )
            critic_grad_fn = jax.value_and_grad(
                mean_critic_loss, has_aux=True
            )
            policy_grad_fn = jax.value_and_grad(
                mean_policy_loss, argnums=(0, 1), has_aux=True
            )
            ...
            for q_update_step in range(self.q_update_steps):
                split_keys = jax.random.split(
                    key, (self.batch_size * 3) + 2
                )
                key = split_keys[0]
                subset_key = split_keys[1]
                sample_keys = split_keys[2::3]
                dropout_current_keys = split_keys[3::3]
                dropout_target_keys = split_keys[4::3]
                target_indices = jax.random.choice(
                    subset_key,
                    jnp.arange(self.ensemble_size),
                    shape=(self.in_target_minimization_size,),
                    replace=False,
                )
                selected_target_params = jax.tree_util.tree_map(
                    lambda value: value[target_indices],
                    critic_state.target_params,
                )
                (_, metrics), critic_gradients = critic_grad_fn(
                    critic_state.params,
                    selected_target_params,
                    states[q_update_step],
                    next_states[q_update_step],
                    raw_next_states[q_update_step],
                    actions[q_update_step],
                    rewards[q_update_step],
                    terminations[q_update_step],
                    sample_keys,
                    dropout_current_keys,
                    dropout_target_keys,
                )
                ...
                critic_grad_norms.append(optax.global_norm(critic_gradients))

            split_keys = jax.random.split(key, (self.batch_size * 2) + 1)
            key = split_keys[0]
            sample_keys = split_keys[1::2]
            dropout_keys = split_keys[2::2]
            (_, policy_metrics), (
                policy_gradients,
                entropy_gradients,
            ) = policy_grad_fn(
            ...
            metrics = {
                name: jnp.mean(
                    jnp.asarray([values[name] for values in critic_metrics])
                )
                for name in critic_metrics[0]
            }
            metrics.update(policy_metrics)
            metrics["lr/learning_rate"] = (
                policy_state.opt_state.hyperparams["learning_rate"]
            )
            metrics["gradients/policy_grad_norm"] = optax.global_norm(
                policy_gradients
            )
            metrics["gradients/critic_grad_norm"] = jnp.mean(
                jnp.asarray(critic_grad_norms)
            )
    ...
    def train(self):
        update = self._build_task_update()

        rlx_logger.info("Compiling Flax DroQ learner kernel before rollout")
        dummy_states = jnp.zeros(
            (self.q_update_steps, self.batch_size)
            + tuple(self.train_env.single_observation_space.shape),
            dtype=jnp.float32,
        )
        dummy_actions = jnp.zeros(
            (self.q_update_steps, self.batch_size)
            + tuple(self.train_env.single_action_space.shape),
            dtype=jnp.float32,
        )
        dummy_scalars = jnp.zeros(
            (self.q_update_steps, self.batch_size), dtype=jnp.float32
        )
        mean, std, epsilon = self._normalizer_parameters()
        warm_result = update(
            self.policy_state,
    ...
    def save(self, model_file_name="best.model"):
        if not hasattr(self.train_env, "checkpoint_manifest"):
            raise ValueError(
                "DroQ-QSafe environments must provide checkpoint_manifest()"
            )
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        if "qsafe_metadata" not in payload:
            raise ValueError(
                "The Flax DroQ-QSafe checkpoint is missing QSafe metadata and "
        ...
        if missing:
            raise ValueError(
                "The Flax DroQ-QSafe checkpoint is missing transfer contract "
        ...
        if not hasattr(train_env, "validate_checkpoint_manifest"):
            raise ValueError(
                "DroQ-QSafe environment must validate the checkpoint manifest"
            )
