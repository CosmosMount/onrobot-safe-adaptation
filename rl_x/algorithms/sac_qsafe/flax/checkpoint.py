...
def convert_torch_task_critic_params(
    template_params,
    first_critic_state: Mapping[str, Any],
    second_critic_state: Mapping[str, Any],
):
    """Convert the two PyTorch SAC Q networks into one vmapped Flax critic."""

    critics = [
        _normalized_state_dict(first_critic_state),
        _normalized_state_dict(second_critic_state),
    ]
    expected = {
        "critic.0.weight",
        "critic.0.bias",
    ...
    for index, critic in enumerate(critics):
        _require_exact_keys(critic, expected, f"PyTorch task critic {index}")

    state = serialization.to_state_dict(template_params)
    params = state.get("params", {}).get("VmapCritic_0", {})
    expected_dense = {"Dense_0", "Dense_1", "Dense_2"}
    if set(params) != expected_dense:
        raise ValueError(
            "Unexpected Flax task critic parameter structure: "
            f"{sorted(params)}"
        )

    for dense_name, torch_prefix in (
        ("Dense_0", "critic.0"),
        ("Dense_1", "critic.2"),
        ("Dense_2", "critic.4"),
    ):
        kernels = np.stack(
            [
                _as_numpy(critic[f"{torch_prefix}.weight"], dtype=np.float32).T
        ...
        biases = np.stack(
            [
                _as_numpy(critic[f"{torch_prefix}.bias"], dtype=np.float32)
                for critic in critics
            ],
            axis=0,
        )
        expected_kernel = np.asarray(params[dense_name]["kernel"]).shape
        expected_bias = np.asarray(params[dense_name]["bias"]).shape
        if kernels.shape != expected_kernel or biases.shape != expected_bias:
            raise ValueError(
                f"{torch_prefix} shape mismatch: expected kernel={expected_kernel}, "
                f"bias={expected_bias}; got kernel={kernels.shape}, bias={biases.shape}"
            )
        params[dense_name]["kernel"] = kernels
        params[dense_name]["bias"] = biases
...
def load_torch_task_artifact(
    ...
    """Load task critics and entropy state from a full PyTorch checkpoint."""
    ...
    if missing:
        raise ValueError(
            f"PyTorch task checkpoint is missing {sorted(missing)}"
        )
    log_alpha = float(_as_numpy(checkpoint["log_alpha"], dtype=np.float32).reshape(()))
    if not np.isfinite(log_alpha):
        raise ValueError("PyTorch task checkpoint log_alpha must be finite")
    return {
        "online_params": convert_torch_task_critic_params(
            online_template,
            checkpoint["q1_state_dict"],
            checkpoint["q2_state_dict"],
        ),
        "target_params": convert_torch_task_critic_params(
            target_template,
            checkpoint["q1_target_state_dict"],
            checkpoint["q2_target_state_dict"],
        ),
        "log_alpha": np.float32(log_alpha),
    }
