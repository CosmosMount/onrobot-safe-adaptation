...
optimizers, and target networks. Full checkpoints retain all learner state for
same-run resume; the target-task transfer bundle contains only the actor,
normalizer, and QSafe.
...
separate policy and QSafe transfer artifacts under the run's `models/`
directory. `best.model` follows task return as before, while `final.model` and
the two transfer sidecars are written unconditionally after the final safety
block so the last QSafe update is never omitted.
...
frozen. Only the actor, observation-normalizer statistics, and QSafe are
transferred from pre-training. The task critics and targets, entropy
...
Eq. 3 action masking and the Eq. 4 actor/dual constraint while preserving the
same actor, normalizer, QSafe checkpoint, fresh task learner state, and random
seed. Both backends optimize the projected non-negative dual variable with Adam
only when QSafe is enabled.
